...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221004001441_FAQs')
BEGIN
    ALTER TABLE [Modules] DROP CONSTRAINT [FK_Modules_Course_CourseID];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221004001441_FAQs')
BEGIN
    EXEC sp_rename N'[Modules].[CourseID]', N'CourseId', N'COLUMN';
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221004001441_FAQs')
BEGIN
    EXEC sp_rename N'[Modules].[IX_Modules_CourseID]', N'IX_Modules_CourseId', N'INDEX';
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221004001441_FAQs')
BEGIN
    ALTER TABLE [Modules] ADD CONSTRAINT [FK_Modules_Course_CourseId] FOREIGN KEY ([CourseId]) REFERENCES [Course] ([CourseId]) ON DELETE CASCADE;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20221004001441_FAQs', N'5.0.17');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221007132434_updateDatabase')
BEGIN
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221007132434_updateDatabase')
BEGIN
    CREATE TABLE [Mark_Capture] (
        [MarkID] int NOT NULL IDENTITY,
        [StudentID] nvarchar(max) NOT NULL,
        [ModuleID] int NOT NULL,
        [Assessment] nvarchar(max) NOT NULL,
        [Assessment_Type] nvarchar(max) NOT NULL,
        [DateOfAssessment] nvarchar(max) NOT NULL,
        [Overall_Mark] int NOT NULL,
        [mark_Obtained] int NOT NULL,
        CONSTRAINT [PK_Mark_Capture] PRIMARY KEY ([MarkID])
    );
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20221007132434_updateDatabase', N'5.0.17');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221007232816_Update_CaptureTable')
BEGIN
    ALTER TABLE [Mark_Capture] ADD [Mark_Percentage] float NOT NULL DEFAULT 0.0E0;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20221007232816_Update_CaptureTable', N'5.0.17');
END;
GO
